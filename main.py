...
class MusicCog(commands.Cog, name="Música"):
    ...
    async def send_music_panel(self, ctx: commands.Context, song: dict):
        """Envía o actualiza el panel de control musical."""
    ...
    def play_next_song(self, ctx: commands.Context):
        """Lógica principal para reproducir la siguiente canción."""
        state = self.get_guild_state(ctx.guild.id)
    ...
    async def play(self, ctx: commands.Context, *, search_query: str):
        ...
        # --- LÓGICA CORREGIDA PARA PLAYLISTS ---
        # Por defecto, no procesa playlists.
        is_playlist = False
        # Si el enlace contiene 'list=', lo tratamos como una playlist.
        if "youtube.com" in search_query and 'list=' in search_query:
            is_playlist = True
        
        # Actualizamos la opción de yt-dlp basándonos en nuestra comprobación.
        ...
        try:
            ...
            await msg.edit(content=f'❌ Ocurrió un error: {e}')
