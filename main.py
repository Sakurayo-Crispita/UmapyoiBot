...
class MusicPanelView(discord.ui.View):
    ...
    async def _execute_command(self, interaction: discord.Interaction, command_name: str):
        ...
        # Crear un contexto falso para ejecutar el comando
        ctx = await self.music_cog.bot.get_context(interaction.message)
        ctx.author = interaction.user
        # Adjuntar la interacción al contexto para que pueda ser respondida
        ctx.interaction = interaction
        
        # Para comandos que envían su propia respuesta, es mejor manejar la respuesta directamente
        # en el comando y no aquí.
    ...
    async def pause_resume_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """
        Pausa o reanuda la canción y actualiza el botón, enviando solo una pequeña confirmación.
        """
        vc = interaction.guild.voice_client
        
        if not vc or not (vc.is_playing() or vc.is_paused()):
            return await interaction.response.send_message("No hay nada para pausar o reanudar.", ephemeral=True, delete_after=10)

        # Realiza la acción de pausar o reanudar directamente aquí
        if vc.is_paused():
            vc.resume()
            confirm_message = "▶️ Canción reanudada."
        else:
            vc.pause()
            confirm_message = "⏸️ Canción pausada."

        # Actualiza el estilo de los botones en el panel original
        self._update_button_styles(interaction)
        await interaction.response.edit_message(view=self)

        # Envía la confirmación corta y temporal que querías
        await interaction.followup.send(confirm_message, ephemeral=True, delete_after=10)
...
class MusicCog(commands.Cog, name="Música"):
    ...
    def handle_after_play(self, ctx: commands.Context, error: Exception | None):
        if error:
            ...
            state = self.get_guild_state(ctx.guild.id)
            # Limpia la canción que falló para evitar inconsistencias
            failed_song_title = state.current_song.get('title', 'la canción anterior') if state.current_song else 'la canción anterior'
            state.current_song = None
            
            # Avisa en el canal del error
            target_channel = ctx.channel or (ctx.interaction and ctx.interaction.channel)
            if target_channel:
                self.bot.loop.create_task(
                    target_channel.send(f"❌ Ocurrió un error al reproducir '{failed_song_title}'. Saltando a la siguiente si hay en la cola.")
                )
        
        # Llama a play_next_song para continuar con la cola o desconectar si está vacía
...
class UtilityCog(commands.Cog, name="Utilidad"):
    ...
    @discord.app_commands.describe(categoría="Elige una categoría para ver sus comandos específicos.")
    async def help(self, ctx: commands.Context, categoría: Optional[str] = None):
        """
        Muestra el panel de ayuda o la ayuda de una categoría específica.
        """
        if categoría is None:
            # Si no se especifica categoría, muestra el panel interactivo
            ...
            # Si se especifica una categoría, busca su nombre real y muestra su ayuda
            cog_name_real = self.cog_map.get(categoría.lower())
            if cog_name_real:
                await self._send_cog_help(ctx, cog_name_real)
            else:
                await ctx.send(f"La categoría '{categoría}' no existe. Usa una de las opciones sugeridas.", ephemeral=True)
    ...
    async def help_autocomplete(self, interaction: discord.Interaction, current: str) -> list[discord.app_commands.Choice[str]]:
        """Ofrece sugerencias de autocompletado para las categorías."""
        choices = [
...
# --- COG DE ECONOMÍA (VERSIÓN FINAL Y COMPLETA) ---
class EconomyCog(commands.Cog, name="Economía"):
    """
    Sistema de economía configurable por servidor.
    Activa la economía en canales específicos y personaliza la experiencia.
    """
    def __init__(self, bot: UmapyoiBot, conn: sqlite3.Connection, lock: asyncio.Lock):
        self.bot = bot
        self.conn = conn
    ...
    def setup_database(self):
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS economy_settings (
                guild_id INTEGER PRIMARY KEY,
                currency_name TEXT DEFAULT 'créditos',
        ...
        ''')
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS economy_active_channels (
                guild_id INTEGER, channel_id INTEGER,
                PRIMARY KEY (guild_id, channel_id)
            )
        ''')
        # Nos aseguramos de que las columnas nuevas existan si la tabla ya fue creada
    ...
    async def get_active_channels(self, guild_id: int) -> list[int]:
        async with self.db_lock:
            ...
            return [row['channel_id'] for row in self.cursor.fetchall()]
    ...
    async def is_economy_active(self, ctx: commands.Context) -> bool:
        ...
        if not active_channels:
            await ctx.send("La economía no está activada en ningún canal. Un administrador debe usar `/economy addchannel`.", ephemeral=True, delete_after=10)
            return False
    ...
    async def log_transaction(self, guild: discord.Guild, author: discord.Member, message: str):
        ...
        if log_channel_id := settings.get('log_channel_id'):
            if channel := guild.get_channel(log_channel_id):
                embed = discord.Embed(description=message, color=CREAM_COLOR, timestamp=datetime.datetime.now())
    ...
    async def update_balance(self, guild_id: int, user_id: int, wallet_change: int = 0, bank_change: int = 0) -> tuple[int, int]:
        async with self.db_lock:
            current_wallet, current_bank = await self.get_balance(guild_id, user_id)
            settings = await self.get_guild_settings(guild_id)
    ...
    async def economy(self, ctx: commands.Context):
        if ctx.invoked_subcommand is None:
            ...
            active_channels = await self.get_active_channels(ctx.guild.id)
            channel_mentions = [f"<#{cid}>" for cid in active_channels] if active_channels else ["Ninguno"]
            max_bal = f"{settings['max_balance']}" if settings['max_balance'] is not None else "Sin límite"
            embed = discord.Embed(title="⚙️ Estado de la Economía", color=CREAM_COLOR)
            embed.description = "Usa los subcomandos de `/economy` para configurar el sistema."
            embed.add_field(name="Canales Activos", value="\n".join(channel_mentions), inline=False)
    ...
    async def add_channel(self, ctx: commands.Context, canal: discord.TextChannel):
        async with self.db_lock:
            self.cursor.execute("INSERT OR IGNORE INTO economy_active_channels (guild_id, channel_id) VALUES (?, ?)", (ctx.guild.id, canal.id))
            self.conn.commit()
    ...
    async def remove_channel(self, ctx: commands.Context, canal: discord.TextChannel):
        async with self.db_lock:
            self.cursor.execute("DELETE FROM economy_active_channels WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, canal.id))
            self.conn.commit()
    ...
    async def set_start_balance(self, ctx: commands.Context, cantidad: int):
        ...
        async with self.db_lock:
            self.cursor.execute("UPDATE economy_settings SET start_balance = ? WHERE guild_id = ?", (cantidad, ctx.guild.id))
            self.conn.commit()
    ...
    async def set_max_balance(self, ctx: commands.Context, cantidad: int):
        ...
        async with self.db_lock:
            self.cursor.execute("UPDATE economy_settings SET max_balance = ? WHERE guild_id = ?", (cantidad, ctx.guild.id))
            self.conn.commit()
    ...
    async def set_audit_log(self, ctx: commands.Context, canal: discord.TextChannel):
        async with self.db_lock:
            self.cursor.execute("UPDATE economy_settings SET log_channel_id = ? WHERE guild_id = ?", (canal.id, ctx.guild.id))
            self.conn.commit()
    ...
    async def reset_economy(self, ctx: commands.Context):
        async with self.db_lock:
            self.cursor.execute("DELETE FROM balances WHERE guild_id = ?", (ctx.guild.id,))
            self.conn.commit()
    ...
    async def withdraw(self, ctx: commands.Context, cantidad: str):
        ...
        await ctx.send(f"💸 Has retirado **{amount_to_withdraw}**. Tu cartera ahora tiene **{new_wallet}**.", ephemeral=True)

    # --- COMANDOS DE USUARIO (INTEGRADOS) ---
    ...
    async def balance(self, ctx: commands.Context, miembro: discord.Member | None = None):
        ...
        target_user = miembro or ctx.author
        settings = await self.get_guild_settings(ctx.guild.id)
        wallet, bank = await self.get_balance(ctx.guild.id, target_user.id)
        embed = discord.Embed(title=f"{settings['currency_emoji']} Balance de {target_user.display_name}", color=CREAM_COLOR)
        embed.add_field(name="Cartera", value=f"`{wallet}`", inline=True)
    ...
    async def work(self, ctx: commands.Context):
        ...
        if retry_after:
            ...
            return await ctx.send(f"Estás cansado. Descansa y vuelve a trabajar en **{int(m)}m {int(s)}s**.", ephemeral=True)
        amount = random.randint(settings['work_min'], settings['work_max'])
        await self.update_balance(ctx.guild.id, ctx.author.id, wallet_change=amount)
        embed = discord.Embed(title="💼 ¡A trabajar!", description=f"Has trabajado duro y ganaste **{amount} {settings['currency_name']}**.", color=discord.Color.green())
        await ctx.send(embed=embed)
    ...
    async def rob(self, ctx: commands.Context, miembro: discord.Member):
        ...
        retry_after = bucket.update_rate_limit()
        if retry_after:
            h, m_total = divmod(retry_after, 3600); m, s = divmod(m_total, 60)
            return await ctx.send(f"Acabas de intentar un robo. Espera **{int(h)}h {int(m)}m**.", ephemeral=True)
        ...
        if random.random() < 0.5:
            amount_stolen = int(victim_wallet * random.uniform(0.1, 0.25))
            await self.update_balance(ctx.guild.id, ctx.author.id, wallet_change=amount_stolen)
            await self.update_balance(ctx.guild.id, miembro.id, wallet_change=-amount_stolen)
            embed = discord.Embed(title="🎭 ¡Robo Exitoso!", description=f"Le has robado **{amount_stolen} {settings['currency_name']}** de la cartera a {miembro.mention}.", color=discord.Color.dark_green())
        else:
            amount_lost = max(50, int(robber_wallet * random.uniform(0.05, 0.15)))
            await self.update_balance(ctx.guild.id, ctx.author.id, wallet_change=-amount_lost)
            embed = discord.Embed(title="🚓 ¡Te Pillaron!", description=f"¡Torpe! {miembro.mention} te vio venir. Perdiste **{amount_lost} {settings['currency_name']}**.", color=discord.Color.dark_red())
        await ctx.send(embed=embed)
    ...
    async def give(self, ctx: commands.Context, miembro: discord.Member, cantidad: int):
        ...
        if sender_wallet < cantidad:
            return await ctx.send(f"No tienes suficientes {settings['currency_name']} en tu cartera. Tienes: **{sender_wallet}**.", ephemeral=True)
        await self.update_balance(ctx.guild.id, ctx.author.id, wallet_change=-cantidad)
        await self.update_balance(ctx.guild.id, miembro.id, wallet_change=cantidad)
        embed = discord.Embed(title="💸 Transferencia Realizada", description=f"{ctx.author.mention} ha transferido **{cantidad} {settings['currency_name']}** a {miembro.mention}.", color=CREAM_COLOR)
        await ctx.send(embed=embed)
    ...
    async def leaderboard(self, ctx: commands.Context):
        ...
        async with self.db_lock:
            ...
            top_users = self.cursor.fetchall()
        if not top_users: return await ctx.send(f"Nadie tiene {settings['currency_name']} en este servidor todavía.")
        embed = discord.Embed(title=f"🏆 Ranking de {settings['currency_name']} 🏆", color=discord.Color.gold())
        description = ""
        for i, user_row in enumerate(top_users):
            try: user = await self.bot.fetch_user(user_row['user_id']); user_name = user.display_name
            except discord.NotFound: user_name = f"Usuario Desconocido ({user_row['user_id']})"
            rank = ["🥇", "🥈", "🥉"][i] if i < 3 else f"`{i+1}.`"
            description += f"{rank} **{user_name}**: {user_row['total']} (Cartera: {user_row['wallet']} / Banco: {user_row['bank']})\n"
        embed.description = description
        await ctx.send(embed=embed)
    # (Aquí añadiremos el resto de comandos más adelante)
